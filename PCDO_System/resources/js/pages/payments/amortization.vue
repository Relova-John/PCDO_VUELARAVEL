...
function canPayPeriod(scheduleId: number) {
	...
	const previousStatus = getStatus(previousSchedule).type

	return ['Paid', 'Partial Paid', 'Overdue'].includes(previousStatus)
}
					...
					<div
						...
						<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
							...
							<div
								...
								<p class="text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400">Start Date
								</p>
							...
							<div
								class="bg-gray-200/50 dark:bg-gray-700 p-4 rounded-xl ring-1 ring-gray-300 dark:ring-gray-700 border-gray-300 dark:border-gray-700 shadow-sm">
								<p class="text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400">Grace Period
								</p>
							...
							<div
								class="bg-gray-200/50 dark:bg-gray-700 p-4 rounded-xl ring-1 ring-gray-300 dark:ring-gray-700 border-gray-300 dark:border-gray-700 shadow-sm">
								<p class="text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400">Loan Term
								</p>
								<p class="mt-1 text-lg font-semibold text-gray-900 dark:text-gray-100">
							...
							<div
								class="bg-gray-200/50 dark:bg-gray-700 p-4 rounded-xl ring-1 ring-gray-300 dark:ring-gray-700 border-gray-300 dark:border-gray-700 shadow-sm">
								<p class="text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400">Expected End
									Date</p>
								...
								(row.data.penalty_amount ?? 0) > 0
									...
									: 'bg-orange-500 hover:bg-orange-600 text-white'
										...
										<TableCell class="px-4 py-3">
											<template v-if="isResolved">
												<div
													class="inline-flex items-center gap-1 text-green-600 italic font-semibold">
													<Check class="w-4 h-4" />
													<span>Resolved</span>
												</div>
											</template>
											...
											<template v-else>
												<div v-if="row.data && !row.data.is_paid && !(row.data.amount_paid && (row.data.balance || 0) > 0)"
													...
													<Button size="sm"
														class="w-36 bg-green-600 hover:bg-green-700 text-white rounded-full"
														@click="openPaymentReceiptModal(row.data!.id)"
														:disabled="!canPayPeriod(row.data!.id)">
														<ReceiptText class="w-3 h-3" /> Mark Paid
													</Button>
													<input type="number"
														v-model.number="scheduleForms[getFormIndex(index)].amount_paid"
													...
													<Button size="sm"
														class="w-36 bg-blue-600 hover:bg-blue-700 text-white rounded-full"
										...
										<TableCell class="px-4 py-3">
											...
											<template
												v-else-if="row.data?.amount_paid && (row.data.balance || 0) > 0 && !row.data.is_paid">
												<div
													class="inline-flex items-center gap-1 text-yellow-600 italic font-semibold">
													<Exclamation class="w-5 h-5" />
													<span>Partially Paid</span>
					...
					<div class="space-y-4 md:hidden">
						<div v-for="(row, index) in allPeriods" :key="index"
							...
							<template v-if="row.type === 'grace'">
								...
								<div
									class="flex items-center justify-center text-yellow-600 dark:text-yellow-300 font-medium">
							...
							<template v-else>
								...
								<div class="space-y-1 text-sm text-gray-600 dark:text-gray-300 mb-3">
									...
									<p><strong>Amount:</strong> ₱{{ Math.round(row.data?.current_balance ||
										0).toLocaleString() }}</p>
								...
								<div class="flex flex-wrap gap-2 justify-between">
									...
									<template v-else>
										...
										<Button size="sm"
											...
											@click="sendNotification(row.data!.id)" :disabled="!canPayPeriod(row.data!.id)">
											🔔 Send Reminder
										</Button>
									</template>
								</div>
						...
						<div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-[90%] max-w-md p-6 relative">
							...
							<div class="flex flex-col gap-3">
								...
								<p class="text-xs text-gray-500 dark:text-gray-400">
									Please upload the official receipt (image or PDF) before marking this loan as
									resolved.
								</p>
							</div>
